      ...
      {projects.map(project => (
        <li key={project.slug} className='group relative'>
          <Link href={`/projects/${project.slug}`}>
            ...
            <div className='absolute inset-x-0 bottom-1 translate-y-2 rounded-lg bg-background/40 px-2 py-2 transition-opacity duration-300 group-hover:hidden'>
              <h2 className='title line-clamp-1 text-center text-xl no-underline'>
                {project.title}
              </h2>
            </div>
          </Link>
